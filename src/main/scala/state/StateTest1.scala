...
object StateTest1 {
  ...
  def main(args: Array[String]): Unit = {
    //要求同一个温度传感器前后两条数据的年龄差不能超过5，否则给一个提示。因此需要将前一条数据保存起来，保存成一个状态，让后一条数据使用。
...
class MyFlatMapper extends RichFlatMapFunction[SensorReading,(Int,Double,Double,Double)]{
  ...
  override def open(parameters: Configuration) = {
    //定义一个Value类型的状态来保存上一条数据的温度值. 在 open方法里完成初始化()
    lastTempState = getRuntimeContext.getState(new ValueStateDescriptor[Double]("last_temperature",classOf[Double]))
